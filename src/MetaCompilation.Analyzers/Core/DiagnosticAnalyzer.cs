        ...
        private class CompilationAnalyzer
        {
            ...
            private void IfDiagnostic(CompilationAnalysisContext context, StatementSyntax statement, DiagnosticDescriptor diagnostic, params string[] messageArgs)
            {
                if (statement.Kind() == SyntaxKind.IfStatement)
                {
                    ...
                    int startDiagnosticSpan = ifStatement.SpanStart;
                    int endDiagnosticSpan = ifStatement.CloseParenToken.SpanStart;
                    TextSpan diagnosticSpan = TextSpan.FromBounds(startDiagnosticSpan, endDiagnosticSpan);
                    Location diagnosticLocation = Location.Create(ifStatement.SyntaxTree, diagnosticSpan);
            ...
            {
                PropertyDeclarationSyntax propertyDeclaration = SuppDiagPropertySymbol(context);
                    ...
                    if (fieldSymbol.Type != null && fieldSymbol.Type.MetadataName == "DiagnosticDescriptor")
                    {
                        ...
                        for (int i = 0; i < ruleArgumentList.Arguments.Count; i++)
                        {
                            ...
                            if (currentArg.NameColon != null)
                            {
                                ...
                                ExpressionSyntax currentArgExpr = currentArg.Expression;
            ...
            private bool CheckMethods(InvocationExpressionSyntax invocationExpression, CompilationAnalysisContext context)
            {
                ...
                if (analysisMethodFound)
                {
                    ...
                    else if (!analysisMethod.Parameters.HasExactly(1) || !Equals(analysisMethod.Parameters.First().Type, context.Compilation.GetTypeByMetadataName("Microsoft.CodeAnalysis.Diagnostics.SyntaxNodeAnalysisContext")))
                    {
                ...
                else
                {
                    ...
                    else if (statements.Count > 1)
                    {
                        ...
                        foreach (ExpressionStatementSyntax statement in statements)
                        {
                            ...
                            string name = expressionStart.Name.Identifier.Text;
            ...
            private BlockSyntax InitializeOverview(CompilationAnalysisContext context)
            {
                ...
                if (!parameters.HasExactly(1) || !Equals(parameters[0].Type, context.Compilation.GetTypeByMetadataName("Microsoft.CodeAnalysis.Diagnostics.AnalysisContext"))

...
namespace Microsoft.NetCore.Analyzers.Performance
{
    ...
    {
        private const string AnyAsyncMethodName = "AnyAsync";

        /// <summary>
        /// A list of diagnostic IDs that this provider can provider fixes for.
        /// </summary>
        /// <value>The fixable diagnostic ids.</value>
        public override ImmutableArray<string> FixableDiagnosticIds { get; } = ImmutableArray.Create(DoNotUseCountAsyncWhenAnyAsyncCanBeUsedAnalyzer.RuleId);
        ...
        public sealed override async Task RegisterCodeFixesAsync(CodeFixContext context)
        {
                ...
                this.TryGetFixer(node, out var expression, out var arguments, out var negate))
            {
                context.RegisterCodeFix(
                    new DoNotUseCountAsyncWhenAnyAsyncCanBeUsedCodeAction(context.Document, node, expression, arguments, negate),
                    context.Diagnostics);
            }
        }
        ...
        private class DoNotUseCountAsyncWhenAnyAsyncCanBeUsedCodeAction : CodeAction
        {
            ...
            public DoNotUseCountAsyncWhenAnyAsyncCanBeUsedCodeAction(Document document, SyntaxNode pattern, SyntaxNode expression, IEnumerable<SyntaxNode> arguments, bool negate)
            {
            ...
            protected override async Task<Document> GetChangedDocumentAsync(CancellationToken cancellationToken)
            {
                ...
                var memberAccess = generator.MemberAccessExpression(this.expression.WithoutTrailingTrivia(), AnyAsyncMethodName);

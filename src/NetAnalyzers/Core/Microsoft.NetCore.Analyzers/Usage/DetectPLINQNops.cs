...
namespace Microsoft.NetCore.Analyzers.Usage
{
    ...
    public sealed class DetectPLINQNops : DiagnosticAnalyzer
    {
        ...
        private static readonly string[] knownLastCalls = new string[] { "ToList", "ToArray" };
        private static readonly LocalizableString s_localizableTitle = new LocalizableResourceString(nameof(MicrosoftNetCoreAnalyzersResources.DetectPLINQNopsTitle), MicrosoftNetCoreAnalyzersResources.ResourceManager, typeof(MicrosoftNetCoreAnalyzersResources));
        ...
        private static void AnalyzeSymbol(SyntaxNodeAnalysisContext context)
        {
            ...
            if (invocation.Parent is ForEachStatementSyntax parentForEach)
            {
                if (parentForEach.Expression.IsEquivalentTo(invocation) || //Last call is AsParallel
                    parentForEach.Expression is MemberAccessExpressionSyntax mem && knownLastCalls.Contains(mem.Name.Identifier.ValueText) //OrToList and ToValue
                    )
                {

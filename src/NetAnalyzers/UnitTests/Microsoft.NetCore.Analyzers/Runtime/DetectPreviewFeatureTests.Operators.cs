...
";
            ...
            test.ExpectedDiagnostics.Add(VerifyCS.Diagnostic(GeneralPreviewFeatureAttributeRule).WithLocation(0).WithArguments("op_UnaryPlus", DefaultURL));
            await test.RunAsync();
        }
        ...
        ";
            var test = TestCS(csInput);
            test.ExpectedDiagnostics.Add(VerifyCS.Diagnostic(GeneralPreviewFeatureAttributeRule).WithLocation(0).WithArguments("op_Addition", DefaultURL));
            await test.RunAsync();
        }
    }
}
